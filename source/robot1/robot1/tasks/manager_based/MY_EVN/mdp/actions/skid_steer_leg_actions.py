...
class SkidSteerLegAction(ActionTerm):
    ...
    def reset(self, env_ids: torch.Tensor | None = None) -> None:
        """
        [关键修复] 当环境重置（机器人摔倒或超时重生）时调用，清除历史指令缓存。
        如果不清除，上一回合摔倒前累积的错误指令会导致新回合刚开始时瞬间被巨大的力矩撕裂。
        """
        if env_ids is None:
            env_ids = slice(None)
        ...
        # 腿部位置缓存【必须】重置为物理引擎中真实的当前关节角度
        # 这样低通滤波器启动时的初始误差为0，避免了第一帧的疯狂抽搐
    ...
    def apply_actions(self):
        ...
        # 第三步：下发给物理引擎底层 (放弃 explicit torque，改用物理引擎内置的隐式 PD，杜绝 NaN 爆炸)
        
        # 轮子：使用底层的速度控制
        # 底层会在刚体配置中根据 damping 自动解算力矩，并截断在 effort_limit 内
