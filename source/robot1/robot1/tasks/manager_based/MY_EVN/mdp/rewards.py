...
def custom_leg_action_rate_l2(env: ManagerBasedRLEnv) -> torch.Tensor:
    """
    专门针对腿部动作的高频抖动惩罚。
    因为动作的前2维是底盘指令(v, omega)，所以我们取第2维之后的部分(腿部指令)进行计算。
    """
    # 获取当前动作和上一步动作
...
def flat_orientation_with_tolerance(
    ...
    """
    带死区的平稳奖励：在 tolerance_deg 范围内不惩罚，超过后施加平方惩罚。
    """
...
class SkidSteerLegRewardsCfg:
    ...
    leg_action_rate_l2 = RewTerm(
        func=custom_leg_action_rate_l2,  # 改为调用上面自定义的函数
        weight=-0.05,                    # 不需要传 asset_cfg
    )
